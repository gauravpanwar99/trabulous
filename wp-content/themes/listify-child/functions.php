...
function gp_enqueue_google_fonts()
{
    ...
    wp_enqueue_style(
        'gp-google-fonts',
        'https://fonts.googleapis.com/css2?family=Jost:wght@300;400;500;600;700&family=Merriweather:wght@300;400;700;900&display=swap',
        [],
        null
    );
}
...
function listify_child_add_custom_listing_data( $data, $listing ) {
    ...
    // Fix: use post_content instead of get_the_content()
...
add_action('wp_footer', function () {
    // Check theme setting
    $show_rating = get_theme_mod('listing-single-rating-checkbox', true);
    // If it's job listing single page and rating is OFF
    if (is_singular('job_listing') && ! $show_rating) {
?>
        <style>
            ...
            ul#select2-search_categories-results li.select2-results__option--highlighted {
                background-color: #84baa0;
            }

            ul#select2-search_categories-results,
            ul#select2-search_categories-results li {
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            function loadCategoriesByRegion(region_id) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'get_categories_by_region',
                        region_id: region_id
                    },
                    success: function(response) {
                        $('#search_categories').html(response);
                    }
                });
            }
            ...
            $('#search_region').on('change', function() {
                ...
                loadCategoriesByRegion(region_id);
            });

            // 2. On Page Load (if already selected)
            let initial_region = $('#search_region').val();
            loadCategoriesByRegion(initial_region);
            ...
            function get_categories_by_region()
            {
                ...
                // -------------------------------------------------
                // CASE 1: region_id = 0 → Return ALL categories WITH POST COUNTS
                // -------------------------------------------------
                if ($region_id === 0) {
